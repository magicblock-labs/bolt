...
import { anchor, web3, FindComponentPda, FindEntityPda, SerializeArgs } from "../../clients/bolt-sdk/lib";
import { Direction } from "../framework";

export function ecs(framework) {
    it("Add entity 1", async () => {
        const world = await framework.worldProgram.account.world.fetch(framework.worldPda);
        framework.entity1Pda = FindEntityPda({ worldId: world.id, entityId: world.entities });
    ...
    it("Add entity 2", async () => {
        const world = await framework.worldProgram.account.world.fetch(framework.worldPda);
        framework.entity2Pda = FindEntityPda({ worldId: world.id, entityId: world.entities });
        const instruction = await framework.worldProgram.methods
          .addEntity(null)
          .accounts({
            payer: framework.provider.wallet.publicKey,
    ...
    it("Add entity 5", async () => {
        const world = await framework.worldProgram.account.world.fetch(framework.worldPda);
        framework.entity5Pda = FindEntityPda({ worldId: world.id, entityId: world.entities });
        const instruction = await framework.worldProgram.methods
          .addEntity(null)
          .accounts({
            payer: framework.provider.wallet.publicKey,
