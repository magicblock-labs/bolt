...
pub mod world {
    ...
    pub fn initialize_component(ctx: Context<InitializeComponent>) -> Result<()> {
        ...
        create_buffer_if_needed(&ctx.accounts.buffer, ctx.bumps.buffer, &ctx.accounts.payer, &ctx.accounts.system_program)?;
        {
...
fn apply_impl<'info>(
    ...
    create_buffer_if_needed(buffer, buffer_bump, authority, system_program)?;
